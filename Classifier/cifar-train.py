...
hyp = {
    ...
    'misc': {
        'ema': {
            'epochs': 10,
            # Slight bug in that this counts only full epochs and then additionally runs the EMA for any fractional epochs at the end too
            'decay_base': .95,
            'decay_pow': 3.,
            'every_n_steps': 5,
        },
...
if not os.path.exists(hyp['misc']['data_location']):
    ...
    num_workers = 0 if os.name == 'nt' else 2  # Windows上设为0，其他系统可设为2
    ...
    eval_dataset_gpu['images'], eval_dataset_gpu['targets'] = [item.to(device=hyp['misc']['device'], non_blocking=True)
                                                               for item in next(iter(eval_dataset_gpu_loader))]

    cifar10_std, cifar10_mean = torch.std_mean(train_dataset_gpu['images'], dim=(0, 2,
                                                                                 3))  # dynamically calculate the std and mean from the data. this shortens the code and should help us adapt to new datasets!
...
def get_whitening_parameters(patches):
    ...
    eigenvalues, eigenvectors = torch.linalg.eigh(est_covariance,
                                                  UPLO='U')  # this is the same as saying we want our eigenvectors, with the specification that the matrix be an upper triangular matrix (instead of a lower-triangular matrix)
...
def init_whitening_conv(layer, train_set=None, num_examples=None, previous_block_data=None, pad_amount=None,
                        freeze=True, whiten_splits=None):
    if train_set is not None and previous_block_data is None:
        if pad_amount > 0:
            previous_block_data = train_set[:num_examples, :, pad_amount:-pad_amount,
                                  pad_amount:-pad_amount]  # if it's none, we're at the beginning of our network.
        else:
    ...
    if whiten_splits is None:
        previous_block_data_split = [
            previous_block_data]  # If we're whitening in one go, then put it in a list for simplicity to reuse the logic below
    else:
        previous_block_data_split = previous_block_data.split(whiten_splits,
                                                              dim=0)  # Otherwise, we split this into different chunks to keep things manageable
    ...
    set_whitening_conv(layer, eigenvalues.to(dtype=layer.weight.dtype), eigenvectors.to(dtype=layer.weight.dtype),
                       freeze=freeze)
...
def make_net():
    ...
    with torch.no_grad():
        ...
        init_whitening_conv(net.net_dict['initial_block']['whiten'],
                            data['train']['images'].index_select(0, torch.randperm(data['train']['images'].shape[0],
                                                                                   device=data['train'][
                                                                                       'images'].device)),
        ...
        for layer_name in net.net_dict.keys():
            if 'conv_group' in layer_name:
                ...
                net.net_dict[layer_name].conv1.weight.data = net.net_dict[
                                                                 layer_name].conv1.weight.data + dirac_weights_in
...
def make_random_square_masks(inputs, mask_size):
    ...
    mask_center_y = torch.empty(in_shape[0], dtype=torch.long, device=inputs.device).random_(mask_size // 2 - is_even,
                                                                                             in_shape[
                                                                                                 -2] - mask_size // 2 - is_even)
    mask_center_x = torch.empty(in_shape[0], dtype=torch.long, device=inputs.device).random_(mask_size // 2 - is_even,
                                                                                             in_shape[
                                                                                                 -1] - mask_size // 2 - is_even)

    # measure distance, using the center as a reference point
    to_mask_y_dists = torch.arange(in_shape[-2], device=inputs.device).view(1, 1, in_shape[-2], 1) - mask_center_y.view(
        -1, 1, 1, 1)
    to_mask_x_dists = torch.arange(in_shape[-1], device=inputs.device).view(1, 1, 1, in_shape[-1]) - mask_center_x.view(
        -1, 1, 1, 1)
...
def batch_crop(inputs, crop_size):
    with torch.no_grad():
        ...
        cropped_batch = torch.masked_select(inputs, crop_mask_batch).view(inputs.shape[0], inputs.shape[1], crop_size,
                                                                          crop_size)
...
def batch_flip_lr(batch_images, flip_chance=.5):
    with torch.no_grad():
        ...
        return torch.where(torch.rand_like(batch_images[:, 0, 0, 0].view(-1, 1, 1, 1)) < flip_chance,
                           torch.flip(batch_images, (-1,)), batch_images)
...
class NetworkEMA(nn.Module):
    ...
    def update(self, current_net, decay):
        with torch.no_grad():
            for ema_net_parameter, (parameter_name, incoming_net_parameter) in zip(self.net_ema.state_dict().values(),
                                                                                   current_net.state_dict().items()):  # potential bug: assumes that the network architectures don't change during training (!!!!)
                if incoming_net_parameter.dtype in (torch.half, torch.float):
                    ema_net_parameter.mul_(decay).add_(incoming_net_parameter.detach().mul(
                        1. - decay))  # update the ema values in place, similar to how optimizer momentum is coded
...
def get_batches(data_dict, key, batchsize, epoch_fraction=1., cutmix_size=None):
    ...
    for idx in range(num_epoch_examples // batchsize):
        if not (
                       idx + 1) * batchsize > num_epoch_examples:  ## Use the shuffled randperm to assemble individual items into a minibatch
            yield images.index_select(0, shuffled[idx * batchsize:(idx + 1) * batchsize]), \
                targets.index_select(0, shuffled[idx * batchsize:(
                                                                             idx + 1) * batchsize])  ## Each item is only used/accessed by the network once per epoch. :D


def init_split_parameter_dictionaries(network):
    params_non_bias = {'params': [], 'lr': hyp['opt']['non_bias_lr'], 'momentum': .85, 'nesterov': True,
                       'weight_decay': hyp['opt']['non_bias_decay'], 'foreach': True}
    params_bias = {'params': [], 'lr': hyp['opt']['bias_lr'], 'momentum': .85, 'nesterov': True,
                   'weight_decay': hyp['opt']['bias_decay'], 'foreach': True}
...
def print_training_details(columns_list, separator_left='|  ', separator_right='  ', final="|", column_heads_only=False,
...
def main():
    ...
    for epoch in range(math.ceil(hyp['misc']['train_epochs'])):
        ...
        # 确定当前epoch的配置
        cutmix_size = hyp['net']['cutmix_size'] if epoch >= hyp['misc']['train_epochs'] - hyp['net'][
            'cutmix_epochs'] else 0
        epoch_fraction = 1.0 if epoch + 1 < hyp['misc']['train_epochs'] else hyp['misc']['train_epochs'] % 1

        # 批次训练
        for epoch_step, (inputs, targets) in enumerate(
                get_batches(data, key='train', batchsize=batchsize, epoch_fraction=epoch_fraction,
            ...
            loss = loss_fn(outputs, targets).mul(hyp['opt']['loss_scale_scaler'] * loss_batchsize_scaler).sum().div(
                hyp['opt']['loss_scale_scaler'])
            ...
            if epoch >= ema_epoch_start and current_steps % hyp['misc']['ema']['every_n_steps'] == 0:
                if net_ema is None:
                    ...
                    continue
                net_ema.update(net, decay=projected_ema_decay_val * (current_steps / total_train_steps) **
        ...
        net.eval()
        eval_batchsize = 2500
        assert data['eval']['images'].shape[0] % eval_batchsize == 0, "Eval batchsize must divide dataset evenly"
        ...
        with torch.no_grad():
            ...
            if ema_val_acc is not None and ema_val_acc > best_ema_acc:
                ...
                best_model_path = os.path.join(model_save_dir, f'best_ema_model_epoch{epoch}_acc{ema_val_acc:.4f}.pth')
                torch.save({
                    'model_state_dict': net_ema.net_ema.state_dict(),
                    'epoch': epoch,
                    'val_acc': ema_val_acc,
                    'hyp': hyp,
                    'optimizer_state': opt.state_dict(),
                    'optimizer_bias_state': opt_bias.state_dict()
                }, best_model_path)
                print(f"Saved new best EMA model with acc {ema_val_acc:.4f} to {best_model_path}")
        ...
        torch.save({
            ...
            'ema_state_dict': net_ema.net_ema.state_dict() if net_ema else None,
            'epoch': epoch,
            'val_acc': val_acc,
            ...
            'optimizer_state': opt.state_dict(),
            'optimizer_bias_state': opt_bias.state_dict(),
            'lr_scheduler_state': lr_sched.state_dict(),
            'lr_scheduler_bias_state': lr_sched_bias.state_dict()
        }, final_model_path)
