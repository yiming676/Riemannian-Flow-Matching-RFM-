...
"""
class Embed128Dataset(Dataset):
    manifold = Sphere()
    dim = 128

    def __init__(self, path):
        assert path.endswith(".csv"), f"Expected CSV path, got {path}"
...
class Embed128Dataset(Dataset):
    ...
    def __init__(self, path):
        ...
        # ✅ 补一维（logdetjac 占位），全 0
        logdetjac = torch.zeros(data.size(0), 1)
        self.data = torch.cat([data, logdetjac], dim=1)
...
class Embed128Poincare(Dataset):
    manifold = PoincareBall()
    dim = 128

    def __init__(self, csv_path, noise_std=0.05, mix_prob=0.5):
        ...
        norms = data.norm(dim=1, keepdim=True).clamp_min(1e-8)
        data = data / norms * 0.999

        data = torch.clamp(data, -0.999, 0.999)
    ...
    def __getitem__(self, idx):
        ...
        if torch.rand(1).item() < self.mix_prob:
            j = torch.randint(len(self.data), (1,)).item()
            x1 = self.data[j]
        else:
            j = torch.randint(len(self.data), (1,)).item()
            lam = torch.rand(1).item()
            x1 = lam * x0 + (1 - lam) * self.data[j]

        noise = self.noise_std * torch.randn_like(x0)
        x0_noisy = x0 + noise

        x0_noisy = torch.clamp(x0_noisy, -0.999, 0.999)
        x1 = torch.clamp(x1, -0.999, 0.999)

        x0_noisy = x0_noisy / (x0_noisy.norm().clamp_min(1e-8)) * 0.99
        x1 = x1 / (x1.norm().clamp_min(1e-8)) * 0.99

        x0_noisy = x0_noisy.float()
        x1 = x1.float()

        assert torch.isfinite(x0_noisy).all(), "x0_noisy has NaN/Inf"
        assert torch.isfinite(x1).all(), "x1 has NaN/Inf"

        return {"x0": x0_noisy, "x1": x1}


class Embed128FlatTorus(Dataset):
    manifold = FlatTorus()
    dim = 128

    def __init__(self, train_path, val_path=None, mode="train"):
        assert mode in {"train", "val"}
        path = train_path if mode == "train" else val_path
        df = pd.read_csv(path, header=None)
        data = torch.tensor(df.values, dtype=torch.float32)
        data = torch.nan_to_num(data, nan=0.0, posinf=0.0, neginf=0.0)
...
class Embed128Spherecifar10(Dataset):
    manifold = Sphere()
    ...
    def __init__(self, path):
        ...
        print(f"Initializing Embed128SphereDataset from: {path}")
        assert path.endswith(".csv"), f"Expected a .csv file path, but got {path}"
        ...
        try:
            ...
            print(f"Could not read CSV with header, trying without. Error: {e}")
            df = pd.read_csv(path, header=None)

        points = torch.tensor(df.values, dtype=torch.float32)

        points = F.normalize(points, p=2, dim=1)

        logdetjac = torch.zeros(points.size(0), 1, dtype=torch.float32)

        self.data = torch.cat([points, logdetjac], dim=1)
...
def _get_dataset(cfg):
    print("[debug] cfg.data =", repr(cfg.data))
    expand_factor = 1
    if cfg.data == "volcano":
        dataset = Volcano(cfg.get("earth_datadir", cfg.get("datadir", None)))
        expand_factor = 1550
    elif cfg.data == "embed128_sphere_cifar10":
        dataset = Embed128Spherecifar10(cfg.embed128_csv_path)
        expand_factor = 4
    elif cfg.data == "embed128_poincare":
        dataset = Embed128Poincare(cfg.embed128_csv_path)
        expand_factor = 4  # 1
    elif cfg.data == "embed128_flattours":
        dataset = Embed128FlatTorus(cfg.embed128_csv_path, cfg.val_csv_path, mode="train")
        expand_factor = 1
    elif cfg.data == "earthquake":
        dataset = Earthquake(cfg.get("earth_datadir", cfg.get("datadir", None)))
        expand_factor = 210
    elif cfg.data == "embed128":
        dataset = Embed128Dataset(cfg.embed128_path)
        expand_factor = 10
    elif cfg.data == "fire":
...
def get_loaders(cfg):
    ...
    val_loader = DataLoader(
        val_set, cfg.optim.val_batch_size, shuffle=False, num_workers=8, pin_memory=True
    )
